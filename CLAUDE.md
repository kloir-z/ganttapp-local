...
- **State Slices** (`src/reduxStoreAndSlices/`):
  - **baseSettingsSlice.ts**: UI settings (widths, heights, language, date ranges). Also holds `extendActualBarToToday` — the project-wide toggle that draws the actual bar up to today for rows that have an actual start but no actual end (default on, saved with the project)
  - **colorSlice.ts**: Task color management and color schemes. The coloring **basis column** (`basisColumnId`: `color` or `textColumn1`〜`textColumn7`) decides which column's value is matched against the palette aliases; each basis column keeps its own palette in `schemes` (the active palette is mirrored into `schemes[basisColumnId]` on every mutation, so the legacy `colors`/`fallbackColor` fields stay valid for all existing consumers). Switching basis (`switchColorBasis`) auto-assigns colors to the column's unique values (golden-angle rgba via `generateAutoColor`; empty alias slots are reused first, capped at 30 auto entries). The built-in palette (`defaultColorValues` + `DEFAULT_FALLBACK_COLOR`) and `generateAutoColor` share one tone — full saturation, 62% lightness, alpha 0.42 — so hand-picked and auto-assigned bars sit at the same weight; keep them in step when changing either (the tutorial sample's palette in `scripts/gen-tutorial-sample.mjs` matches too); the actual-bar color (id 999) is shared across all schemes. Palette entries are dynamic (`addColorInfo`/`removeColorInfo`). Pure helpers `getColorSourceValue`/`resolveColorFromPalette`/`collectBasisValues` are shared by the chart components and the Excel export. The basis is switchable from three places (all via the `useColorBasis` hook): the top-bar "Coloring" menu, right-click > "Color Basis", and the selector in Chart Setting
  - **copiedRowsSlice.ts**: Copy/paste functionality for rows
  - **notesSlice.ts**: Rich text notes storage and management. Also holds `showToolbar` — the shared "is the Quill formatting toolbar pinned open" preference. It is a UI preference, not project data: `setShowToolbar` persists it to `localStorage` (`gantty_notes_toolbar_visible`, default off) and deliberately does **not** set `isSavedChanges`, and neither `resetNotes` nor `setNotes` touch it
...
- **Grid Utilities** (`src/components/Table/utils/`):
  - **gridHandlers.ts**: ReactGrid change handlers. Planned start/end changes arriving together (Excel-like range paste) are merged per row and then dispatched as a **single `setPlannedDates`** — dispatching per row would let each row's dependency recalculation overwrite the rows pasted before it (the pasted block collapsing into one repeated value). Rows the paste covered but whose value didn't change are added at their current values via the `pastedRowIds` argument (ReactGrid emits no change for them, so without this they get dragged along by a neighbour's dependency cascade). Planned dates are applied *after* `setEntireData` so a mixed paste (dates + other columns) doesn't clobber them; `pushPastState` is skipped when `setEntireData` already pushed the undo snapshot (unit-tested in `gridHandlers.test.ts`). Incoming `customDate` cells are first normalized by `pastedDateResolver.ts` (year-less input such as `7/5` gets a year from the surrounding rows), which is why the handler takes the project's `dateFormat`
  ...
  - **pastedDateResolver.ts**: Pure resolver that turns pasted date text into the canonical `yyyy/MM/dd`, borrowing a year when the input omits it (`7/5`). Year sources, in order: the same row's start date (for end-date columns) → the nearest row above/below in the same column (values resolved earlier in the same paste count) → any other date on the row → today's year. Text that states a year always wins. A year-less **end** date that would land before its own row's start date rolls over to the next year (start `2026/11/5` + end `1/10` → `2027/1/10`); no other rollover happens, so a column of dates keeps the neighbouring year. `handleGridChanges` runs every `customDate` change through it before its existing logic, so both the paste and the typed-input paths end up with year-qualified dates. Unit-tested in `pastedDateResolver.test.ts`
  - **clipboardCopy.ts**: Pure builders for the copy payload — `buildClipboardTsv` (rows joined with CRLF, cells with tabs, Excel-style quoting when a cell holds a tab/newline/quote) and `buildClipboardHtml` (ReactGrid's own `data-reactgrid="reactgrid-content"` table with each cell's JSON, so pasting back into the app resolves cell types exactly as before). `cellToClipboardText` renders a cell as it appears on screen — dates follow the `showYear`/`dateFormat` settings, checkboxes become `TRUE`/`FALSE`. Unit-tested in `clipboardCopy.test.ts`
  - **wbsHelpers.ts**: WBS data manipulation utilities. `parseFlexibleDate` interprets loosely-typed/pasted dates (`7/5`, `2026-8-4`, `20260804`, `2026年8月4日`, full-width digits) against the project's `dateFormat` with a fallback year; `hasExplicitYear` reports whether the input stated a year
...
- **Notes System** (`src/components/Topbar/Notes/`):
  ...
  - **QuillEditor.tsx**: Rich text editor with Quill.js integration. Shared by the notes modal and the row-note popovers. The formatting toolbar is **collapsed by default** and toggled by the small "Aa" button rendered at the editor's top-right (same faint-until-hover treatment as the zoom controls at the bottom-right); state lives in `notes.showToolbar`, so toggling in one editor affects them all. Collapsing is done purely in CSS (`.quill-toolbar-hidden .ql-toolbar { display: none }` in `quill.css`) — re-creating the Quill instance would clear the editor's contents. The hidden rule also restores `.ql-container`'s top border, which Quill drops whenever a toolbar is present, and `.ql-toolbar` carries a `padding-right` so its last button never sits under the Aa button. `readOnly` (history view) builds no toolbar at all, so the Aa button is not rendered either
...
- **Additive fields**: `extendActualBarToToday` (ongoing-actual-bar toggle) is written alongside the other settings and read back with a `!== false` default, so files that predate it load with the feature on; no schema bump was needed since it is an optional boolean
...
- **CommonUtils.ts**: Core business logic for date calculations, dependency resolution, holiday handling, and row operations. `resolveActualEndDate(start, end, extendToToday, today)` is the shared rule for the "ongoing actual bar": an empty actual end falls back to today (clamped to the start date when the start is in the future) so a task in progress stays visible. Chart rendering (`ChartRowComponent`) and the Excel export both go through it, and the stored dates are never rewritten — dragging such a bar is what fixes its end date
...
- **useRangeClipboardCopy.ts**: Tab-separated copy — ReactGrid writes the copy table's `textContent` (no separators at all) into `text/plain`, so pasting into Notepad or any plain-text app produced one long line. This hook intercepts `copy`/`cut` in the capture phase on the WBS container (same `.rg-hidden-element` guard as `useRangePasteFill`) and writes both formats itself via `clipboardCopy.ts`: TSV for the outside world, ReactGrid-format HTML for pasting back into the app. On `cut` it re-dispatches a synthetic `cut` so ReactGrid still clears the cells, with `navigator.clipboard.write` neutered for that dispatch (ReactGrid would otherwise overwrite the clipboard asynchronously). WBSInfo supplies the active range's cells from the same `onSelectionChanged` bookkeeping the paste hook uses. Because of this hook, WBSInfo's Ctrl+C handler (whole-row selection → row copy into Redux) no longer calls `preventDefault`, so a full-row copy now also lands on the clipboard as TSV
- **useRangePasteFill.ts**: Excel-like range paste — intercepts the `paste` event in the capture phase on the WBS container (only when the target is ReactGrid's `.rg-hidden-element`, so cell-editor pastes are untouched), tiles the clipboard to the active selection via `pasteTiling.ts`, and re-dispatches a synthetic `ClipboardEvent` carrying the enlarged payload. ReactGrid then sees a clipboard that already matches the selection, so cell-type resolution, `onCellsChanged`/`handleGridChanges` and the single undo snapshot stay unchanged. WBSInfo feeds it the active range (row ids + column count) from `onSelectionChanged`, and its `onPaste` callback reports the pasted range back — WBSInfo hands those row ids to `handleGridChanges` so dependency recalculation can't move rows the paste covered
