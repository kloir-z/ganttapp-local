...
- **useGanttExcelExport.ts**: Excel export — gathers Redux state (including the `notes` slice's `treeData`/`noteData`/`rowNoteData` for the Notes sheet) and downloads the workbook from `GanttExcelExport.ts`
