...
- **Excel（表示専用）**: 画面の見た目を `.xlsx` として再現します。**アプリと同等の機能があるわけではなく、Excel上で「表示するだけ」のものです**。具体的には:
  - 左側にWBS表（表示中の列。画面の `No` 列は `1` / `1-1` / `1-1-1` のような機械的な階層WBS番号に置き換わります）、右側に1日＝1列のチャートを並べます。
  - 計画／実績バー、土日祝の塗り、セパレータ行の帯、月初の罫線などの「見た目」を再現します。
...
- **単体HTML**: アプリ本体と現在のデータを1ファイルに埋め込んだHTMLを書き出します。
  ...
  - **注意点**:
    - `file://` で正しく動くのは、**ライブサイト（GitHub Pages）または単一HTMLビルド（`build:singlefile`）から書き出した場合**です。開発サーバー `npm run dev` からの出力は `type="module"` 形式のため `file://` では動きません。
    - 編集してもファイル自身には保存されません。変更を残すには、開いた先で改めてZIPまたは単体HTMLとして書き出してください。
