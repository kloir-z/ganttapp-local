...
- **単体HTMLに出力**: 「ファイル」→「単体HTMLファイルに出力」で、アプリ本体と現在のデータを1ファイルに埋め込んだHTMLを書き出します。受け取った人がダブルクリックで開くだけで、完成したガントチャートがそのまま表示され、スクロール・編集・メモ閲覧も可能です（サーバー不要）。単体HTMLビルド（`build:singlefile`）から実行すると `file://` で完全に動作します。
